...
const DEFAULT_IMAGE = "https://images.unsplash.com/photo-1550989460-0adf9ea622e2?q=80&w=1000";
...
export function CategoryGrid({ categories }: CategoryGridProps) {
  ...
  const sortedCategories = [...categories].slice(0, 5);
  ...
  // 1. SLATE & CO (TECH MINIMALIST 5-COLUMN GRID)
  if (brandKey.includes("slate")) {
    return (
      <section className="container px-6 py-12 mx-auto font-sans">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8">
          <div className="space-y-1">
            <span className="text-[8px] font-bold uppercase tracking-[0.25em] text-accent">Active Modules</span>
            <h2 className="text-2xl font-bold uppercase tracking-tight text-primary">System Index</h2>
          </div>
          <Link href="/shop" className="text-[10px] font-bold tracking-widest uppercase text-accent flex items-center hover:opacity-80 transition-opacity">
            Full Registry <ArrowRight className="ml-1.5 h-3.5 w-3.5" />
          </Link>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {sortedCategories.map((category) => {
            const image = category.image_url || (category as any).image || FALLBACK_IMAGES[category.slug] || DEFAULT_IMAGE;
            return (
              <Link key={category.id} href={`/shop?category=${category.slug}`} className="group flex flex-col gap-3">
                <div className={cn("aspect-[4/5] w-full overflow-hidden border border-border/10 bg-secondary/15 relative transition-all duration-300 group-hover:border-accent/40", cardRadiusClass)}>
                  <Image
                    src={image}
                    alt={category.name}
                    fill
                ...
                <div className="space-y-0.5 px-1 font-sans">
                  ...
                  <p className="text-[8px] text-muted-foreground font-mono">SYS.LOC // ACCESS SHOP</p>
                </div>
              </Link>
            );
          })}
  ...
  // 2. L'ARTELIER (STARK BLACK-WHITE EDITORIAL TABLE ROW)
    ...
    return (
      <section className="container px-6 py-16 mx-auto font-serif text-black bg-white border-t border-black">
        <div className="flex justify-between items-end mb-12">
          <div>
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-400">02 / DEPARTMENTS</span>
            <h2 className="text-4xl font-black uppercase tracking-tighter text-black mt-1">THE COLLECTION REGISTRY</h2>
          </div>
          <Link href="/shop" className="text-[10px] font-bold tracking-[0.25em] uppercase text-black hover:underline underline-offset-4 decoration-2">
            EXPLORE CATALOGUE <ArrowRight className="inline-block ml-1 h-3.5 w-3.5" />
          </Link>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-5 gap-0 border-t-2 border-l-2 border-black">
            ...
            return (
              ...
              >
                ...
                <div className="z-20 flex justify-between items-start font-sans font-bold text-xs text-black group-hover:text-white transition-colors duration-300">
                  <span>0{index + 1}.</span>
                  <span className="text-[9px] uppercase tracking-widest bg-zinc-100 group-hover:bg-transparent border border-black group-hover:border-white px-2 py-0.5">COLLECTION</span>
                </div>

                <div className="z-20 space-y-2 mt-auto">
                  <h3 className="text-xl font-black uppercase tracking-tighter text-black group-hover:text-white transition-colors duration-300">
                    {category.name}
                  </h3>
                  <span className="inline-flex items-center gap-1.5 font-sans font-bold text-[9px] tracking-widest uppercase text-accent group-hover:text-white transition-all opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0">
                    SHOP ARCHIVES <ArrowRight className="h-3 w-3" />
                  </span>
                </div>
              </Link>
            );
  ...
  // 3. OASIS CO (WARM EARTH ARTISANAL POTTERY CELLS)
  if (brandKey.includes("oasis") || brandKey.includes("sandstone")) {
    return (
      <section className="container px-6 py-12 mx-auto font-serif">
        <div className="text-center space-y-2 mb-12">
          <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-[#D97706]">Earthy Foundations</span>
          <h2 className="text-3xl font-light text-[#402014] uppercase tracking-wide">Artisanal Chambers</h2>
        </div>
          ...
          {sortedCategories.map((category) => {
            ...
            return (
              <Link key={category.id} href={`/shop?category=${category.slug}`} className="group flex flex-col gap-4 text-center">
                <div className="aspect-[3/4] w-full overflow-hidden bg-white border border-[#402014]/10 rounded-[2.5rem] relative shadow-md transition-all duration-500 group-hover:-translate-y-1 group-hover:shadow-lg">
                  <Image
                    src={image}
                    alt={category.name}
                    fill
                    ...
                    sizes="(max-width: 768px) 100vw, 20vw"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#402014]/30 to-transparent" />
                </div>
                <div>
                  <h3 className="text-base font-light text-[#402014] uppercase tracking-wide">{category.name}</h3>
                  <span className="font-sans text-[9px] font-semibold uppercase tracking-widest text-[#D97706] hover:underline">Explore Chamber</span>
                </div>
              </Link>
            );
          })}
  ...
  // 4. OCEAN MIST (COASTAL CAPSULE SWIPER ROW)
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="space-y-1">
            <span className="inline-flex items-center gap-1.5 text-cyan-600 text-[9px] font-bold uppercase tracking-widest">
              <Compass className="h-3 w-3 animate-spin-slow" /> ACTIVE HABITATS
            </span>
            <h2 className="text-2xl font-black text-cyan-950 uppercase tracking-tight">Active Divisions</h2>
          </div>
          <Link href="/shop" className="text-[10px] font-bold tracking-[0.2em] uppercase text-[#14B8A6] flex items-center hover:opacity-80 transition-opacity">
            Registry Index <ArrowRight className="ml-1.5 h-3.5 w-3.5" />
          </Link>
        </div>
  ...
  // 5. DEFAULT: SCENTED (ASYMMETRIC GRID RETAINED)
  return (
    <section className="container px-6 py-12 mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 md:grid-rows-2 gap-4 h-auto md:h-[500px]">
        {sortedCategories.slice(0, 5).map((category, index) => {
          // Custom layout: 1st is large, rest are small
          ...
          return (
            <Link
              ...
              href={`/shop?category=${category.slug}`}
              className={cn(
                "group relative overflow-hidden bg-secondary/10 flex flex-col justify-end border border-border/10 transition-all duration-500 hover:shadow-xl ambient-glow h-64 md:h-auto",
                cardRadiusClass,
